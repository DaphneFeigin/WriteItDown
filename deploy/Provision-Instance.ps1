...
aws ec2 run-instances --image-id $ami --iam-instance-profile Arn=$instanceProfileArn --security-group-ids $sgId --user-data $cloudInitFile --instance-type t2.micro --subnet-id $subnetId --key-name $keyName